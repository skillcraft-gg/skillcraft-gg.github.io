...
const sortPinnedFirst = (left: CredentialDefinition, right: CredentialDefinition) => {
...
export default function CredentialsList({ credentials, owners }: CredentialsListProps) {
  ...
  const sortedCredentials = useMemo(() => {
    ...
    if (state.sort === 'name') {
      clone.sort((a, b) => sortPinnedFirst(a, b) || a.name.localeCompare(b.name))
    } else if (state.sort === 'owner') {
      clone.sort((a, b) => {
        const pinnedSort = sortPinnedFirst(a, b)
        if (pinnedSort !== 0) {
          return pinnedSort
        }
      ...
      clone.sort((a, b) => sortPinnedFirst(a, b) || (parseUpdatedAt(b.updatedAt) - parseUpdatedAt(a.updatedAt)))
    }
            ...
            >
              <div className={`item-grid ${isListView ? 'item-grid--list' : 'item-grid--credentials'}`}>
                {results.map((credential) => {
                  const isPinnedCredential = credential.id === PINNED_CREDENTIAL_ID
